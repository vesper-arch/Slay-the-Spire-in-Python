...
class Player:
    ...
    def __init__(self, health: int, block: int, max_health: int, energy: int, max_energy: int, deck: list):
        self.health = health
        self.block = block
        self.name = "Ironclad"
    ...
    def use_defend(self):
        ...
        if self.frail > 0:
            ...
            ansiprint(
                f"{self.name} gained <red>{math.floor(cards['Defend']['Block'] * 0.75)}</red> <light-cyan>Block</light-cyan> | Block was reduced by <light-cyan>Frail</light-cyan>")
        else:
            self.blocking(cards['Defend']["Block"])
            print(
                f"Player gained <blue>{cards['Defend']['Block']} Block</blue>")
    ...
    def draw_cards(self):
        if len(player.draw_pile) < 5:
            player.draw_pile.extend(random.sample(
                player.discard_pile, len(player.discard_pile)))
            player.discard_pile = []
        ...
        while True:
            if action == "Remove":
                ...
                for using_card in player.deck:
                    ansiprint(
                        f"{counter}: <blue>{using_card['Name']}</blue> | <light-red>{using_card['Energy']} Energy</light-red> | <yellow>{using_card['Info']}</yellow>")
                    counter += 1
                try:
...
class Enemy:
    ...
    def enemy_turn(self):
        ...
        if self.name == 'Spheric Guardian':
            if combat_turn == 1:
                ...
                damage(10, player, self)
                player.frail += 5
                ansiprint(
                    f"{self.name} dealt 10 damage to player and inflicted 5 <yellow>Frail</yellow>(Recieve 25% less block from cards)")
                sleep(2)
                system("clear")
                combat_turn += 1
    ...
    def attack(self, dmg, times, start, end, name=""):
        ...
        for i in range(times):
            ...
            print(f"{self.name} dealt {dmg} damage to {player.name}")
            sleep(0.5)
        if end is True:
            sleep(1)
            system("clear")
...
# Enemies
Blue_Slaver = Enemy(48, 48, 0, "Blue Slaver", ["0%40", "1%60"], [])
Blue_Slaver.moves = [[(Blue_Slaver.attack, 12, 1, True, True, "Stab")], # Stab: Deal 12 damage
                     [(Blue_Slaver.attack, 7, 1, True, False, "Rake")], [(Blue_Slaver.debuff, "Weak", 1, False, True)]] # Rake: Deal 7 damage. Apply 1 Weak
encounters = [[Blue_Slaver]]
